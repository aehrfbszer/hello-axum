...
pub fn make_jwt_token(user_id: &str) -> Result<String, AppError> {
    ...
    encode(&Header::new(Algorithm::EdDSA), &claims, &KEYS.encoding)
        .map_err(|_| AppError::Generic("Failed to create JWT token"))
}
    ...
    B: Send + Into<axum::body::Body>,
{
    ...
    let token = req
        ...
        .map(|t| t.trim())
        .filter(|t| !t.is_empty())
        .ok_or_else(|| AppError::AuthenticationError("InvalidToken").into_response())?;

    // Validate and decode the token.
    let token_data = decode::<Claims>(token, &KEYS.decoding, &Validation::new(Algorithm::EdDSA))
        .map_err(|err| {
            tracing::error!("Error decoding token: {:?}", err);
            AppError::AuthenticationError("InvalidToken").into_response()
        })?;
