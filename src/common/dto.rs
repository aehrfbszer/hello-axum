    ...
    T: Serialize,
{
    ...
    /// Create a failure response with a status code, message, and optional data.
    pub fn failure_with_detail(status: u16, message: impl Into<String>, data: Option<T>) -> Self {
        Self {
            status,
            ok: false,
            message: message.into(),
    ...
    /// Create a failure response with a status code and message.
...
pub type ApiResult<T> = std::result::Result<RestApiResponse<T>, RestApiResponse<()>>;
